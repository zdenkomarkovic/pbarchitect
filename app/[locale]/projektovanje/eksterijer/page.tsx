...
export async function generateMetadata({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: "eksterijer" });
  return buildMetadata({ title: t("metaTitle"), description: t("metaDesc"), locale, localePath: "/projektovanje/eksterijer" });
}

export default async function EksterijerPage({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "eksterijer" });
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid grid-cols-1 gap-16 lg:grid-cols-2">
            <div className="space-y-6">
              ...
              <div>
                ...
                <ul className="space-y-2 text-[#6b6b6b]">
                  {[t("scope1"), t("scope2"), t("scope3"), t("scope4"), t("scope5"), t("scope6"), t("scope7")].map((item) => (
            ...
            <div className="relative aspect-video w-full overflow-hidden rounded-lg">
              <Image src="/dvoriste.webp" alt={t("imgAlt")} fill className="object-cover" />
            </div>
          </div>
        </div>
      </section>
