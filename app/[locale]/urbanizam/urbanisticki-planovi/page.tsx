...
export async function generateMetadata({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: "urbanistickiPlanovi" });
  return buildMetadata({ title: t("metaTitle"), description: t("metaDesc"), locale, localePath: "/urbanizam/urbanisticki-planovi" });
}

export default async function UrbanistickiPlanoviPage({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "urbanistickiPlanovi" });
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid grid-cols-1 gap-16 lg:grid-cols-2">
            <div className="space-y-6">
              ...
              <div>
                ...
                <ul className="space-y-2 text-[#6b6b6b]">
                  {[t("type1"), t("type2"), t("type3"), t("type4")].map((item) => (
            ...
            <div className="relative aspect-video w-full overflow-hidden rounded-lg">
              <Image src="/urbanizam.webp" alt={t("imgAlt")} fill className="object-cover" />
            </div>
          </div>
        </div>
      </section>
